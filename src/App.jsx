      ...
      <main className="main-content">
        ...
        {loading ? <LoadingState /> : cases.length === 0 && !error ? (
          ...
          <>
            ...
            {tab === "audit" && <AuditTrailRebuild cases={cases} demo={demo} doAction={doAction} />}
...
function IncidentWorkspaceRebuild({ detail, doAction, demo }) {
  ...
  return <section className="incident-workspace">
    ...
    <section className={`recommendation-card recommendation-sticky panel ${detail.lifecycle === "RECOVERED" || demoRecovered ? "recovered" : ""}`}><div className="recommendation-copy"><div className="section-kicker"><Zap size={14} /> RECOMMENDED ACTION</div><h3>{detail.lifecycle === "RECOVERED" || demoRecovered ? "Recovery receipt" : "A bounded recovery is ready."}</h3><p>{detail.diagnosis_summary || detail.recommendation?.label || ACTION_LABEL[top?.code] || "Operator review required before execution."}</p><div className="safe-note"><ShieldAlert size={14} />Why safe: {policy.reversible === false ? "bounded policy with operator oversight" : "reversible policy with bounded retries"}.</div></div><div className="recommendation-outcome"><div className="outcome-grid"><span>CONFIDENCE<strong>{Math.round((agentData?.confidence || top?.confidence || 0) * 100)}%</strong></span><span>RECOVERABLE REVENUE<strong>{formatCurrency(demoRecovered ? demo.recoveredAmount : detail.recovered_amount || projection.recoverable)}</strong></span><span>EXPECTED RATE<strong>{Math.round((projection.expected_recovery_rate || 0) * 100)}%</strong></span><span>AFFECTED TRANSACTIONS<strong>{demoRecovered ? demo.recoveredTx : detail.recovered_tx || detail.window_failed}</strong></span><span>RETRY LIMIT<strong>{policy.retry_limit ?? "—"}</strong></span><span>REVERSIBLE POLICY<strong>{policy.reversible === false ? "No" : "Yes"}</strong></span></div>{detail.status === "pending" && !demoRecovered && <div className="operator-actions"><button className="action-button approve" style={btnStyle(C.green, true)} onClick={() => doAction(detail.id, "approve")}><CheckCircle2 size={15} />Approve Recovery</button><button className="action-button reject" style={btnStyle(C.red)} onClick={() => doAction(detail.id, "reject")}><ShieldAlert size={15} />Escalate Investigation</button></div>}{detail.status === "abstained" && <div className="abstain-block"><AlertTriangle size={16} /><div><strong>Confidence below the {Math.round(CONFIDENCE_FLOOR * 100)}% automation floor.</strong><span>{detail.abstain_reason || "No automated recovery was attempted. This case was escalated for manual review instead of guessing."}</span><div style={{ display: "flex", gap: "10px", marginTop: "6px", flexWrap: "wrap" }}><button className="text-button" onClick={() => doAction(detail.id, "llm-review")}><BrainCircuit size={13} />{detail.llm_opinion ? "Refresh AI second opinion" : "Get AI second opinion"}</button><button className="text-button" onClick={() => doAction(detail.id, "mark-reviewed")}>Mark reviewed by ops <ArrowUpRight size={13} /></button></div></div></div>}{detail.llm_opinion && (<div className="abstain-block" style={{ borderColor: detail.llm_opinion.available ? "#7caeff" : "#ff718b", marginTop: "10px" }}><BrainCircuit size={16} /><div><strong>AI second opinion — advisory only, not an automated decision</strong>{detail.llm_opinion.available ? (<><span>{detail.llm_opinion.agrees_with_abstention ? "Agrees the evidence is genuinely ambiguous." : `Suggests ${CODE_LABEL[detail.llm_opinion.likely_cause] || detail.llm_opinion.likely_cause} (${detail.llm_opinion.confidence_percent}% confidence) — still requires manual review.`}</span><span style={{ display: "block", marginTop: "4px" }}>{detail.llm_opinion.reasoning}</span><span style={{ display: "block", marginTop: "4px", opacity: 0.8 }}><b>Suggested next step:</b> {detail.llm_opinion.suggested_next_step}</span></>) : (<span>Unavailable — {detail.llm_opinion.error}</span>)}</div></div>)}{(detail.status === "approved" || demoRecovered) && <div className="outcome-block"><Check size={16} /><div><strong>Recovered {formatCurrency(demoRecovered ? demo.recoveredAmount : detail.recovered_amount)}</strong><span>{demoRecovered ? demo.recoveredTx : detail.recovered_tx} retries completed · audit trail written</span>{!demoRecovered && <button className="text-button" onClick={() => doAction(detail.id, "revert")}><RotateCcw size={13} />Revert recovery</button>}</div></div>}</div></section>
...
function AuditTrailRebuild({ cases, demo, doAction }) {
  ...
  useEffect(() => { if (selectedId) fetch(`${API_BASE}/api/cases/${selectedId}`).then((res) => res.json()).then((data) => { setDetail(data); setEventIndex(0); setExpanded(0); }); }, [selectedId]);
  const events = detail?.timeline || [];
  const filterEvents = (event) => { const text = event.event.toLowerCase(); if (filter === "decisions") return /diagnos|recommend|root cause|policy/.test(text); if (filter === "operator") return /operator|manual/.test(text); if (filter === "recoveries") return /recover|outcome|approved/.test(text); if (filter === "escalations") return /abstain|escalat|reject/.test(text); return true; };
  const visibleEvents = events.filter(filterEvents);
  async function replay() { if (replaying) return; setReplaying(true); for (let i = 0; i < visibleEvents.length; i += 1) { setEventIndex(i); setExpanded(i); await new Promise((resolve) => setTimeout(resolve, 750)); } setReplaying(false); }
  const selected = cases.find((item) => item.id === selectedId) || cases[0];
  return <div className="replay-page"><section className="replay-hero panel"><div><div className="section-kicker"><FileClock size={14} /> DECISION REPLAY CENTER</div><h2>Every recovery has a memory.</h2><p>Replay the complete chain of evidence, reasoning, operator approval, and recovery outcome.</p></div><div className="replay-hero-meta"><span className="confidence-chip">LIVE LOG</span><span className="confidence-chip">IMMUTABLE SESSION</span><small>Batch #{String(selected?.id || 1).padStart(4, "0")}</small></div></section><div className="replay-layout"><aside className="session-list panel"><div className="section-kicker">SESSION HISTORY</div><h3>Decision replay</h3><div className="session-group">TODAY</div>{cases.map((item) => <button className={`session-item ${item.id === selectedId ? "selected" : ""}`} key={item.id} onClick={() => setSelectedId(item.id)}><div><strong>{item.segment}</strong><small>{formatCurrency(item.recovered_amount || 0)} recovered</small></div><LifecycleChip state={item.lifecycle} /></button>)}<div className="session-group">YESTERDAY</div><div className="session-empty">No earlier sessions in this batch.</div></aside><main className="replay-main"><div className="replay-controls"><div className="audit-filters">{[["all", "All Events"], ["decisions", "AI Decisions"], ["operator", "Operator Actions"], ["recoveries", "Recoveries"], ["escalations", "Escalations"]].map(([key, label]) => <button className={filter === key ? "active" : ""} key={key} onClick={() => { setFilter(key); setEventIndex(0); }}>{label}</button>)}</div><div className="audit-search"><Search size={14} /><input placeholder="Search audit ID" value={query} onChange={(e) => setQuery(e.target.value)} /></div></div><section className="replay-timeline-card panel"><div className="replay-section-heading"><div><div className="section-kicker"><Activity size={14} /> DECISION REPLAY</div><h3>Replay Timeline</h3></div><button className="action-button demo-button" onClick={replay} disabled={replaying}><Play size={13} />{replaying ? "Replaying…" : "Replay Investigation"}</button></div><div className="replay-timeline">{visibleEvents.filter((item) => !query || item.event.toLowerCase().includes(query.toLowerCase())).map((item, index) => <ReplayEvent key={`${item.ts}-${index}`} item={item} index={index} active={index === eventIndex} expanded={index === expanded} onClick={() => { setEventIndex(index); setExpanded(expanded === index ? -1 : index); }} />)}</div></section><EvidenceSnapshot detail={detail} event={visibleEvents[eventIndex]} /><DecisionSummary detail={detail} /><RecoveryReceipt detail={detail} demo={demo} doAction={doAction} /></main></div></div>;
}

function ReplayEvent({ item, active, expanded, onClick }) { const text = item.event; const title = /detect/i.test(text) ? "Detection" : /diagnos|root cause/i.test(text) ? "Diagnosis Generated" : /recommend|policy/i.test(text) ? "Recovery Recommendation" : /approv|operator/i.test(text) ? "Operator Approval" : /recover|outcome/i.test(text) ? "Revenue Recovered" : /evidence|baseline/i.test(text) ? "Evidence Aggregation" : "Investigation Event"; return <button className={`replay-event ${active ? "active" : ""}`} onClick={onClick}><span className="replay-event-node"><Activity size={13} /></span><div className="replay-event-card"><div className="replay-event-top"><small>{new Date(item.ts).toLocaleTimeString([], { hour12: false })}</small><strong>{title}</strong><span>{expanded ? "−" : "+"}</span></div><p>{text}</p>{expanded && <div className="replay-event-detail">Event preserved in the immutable session log. Case evidence and operator context were captured at this point in the investigation.</div>}</div></button>; }

function EvidenceSnapshot({ detail, event }) { if (!detail) return null; const top = detail.hypotheses?.[0]; return <section className="audit-evidence-board panel"><div className="section-kicker"><BarChart3 size={14} /> EVIDENCE SNAPSHOT</div><h3>Evidence at this moment</h3><div className="audit-evidence-grid"><ForensicCard title="Failure Codes" value={top?.code || "—"} caption={`${top?.share || 0} occurrences`} /><ForensicCard title="Affected Gateway" value={detail.segment || "—"} caption="Primary payment segment" /><ForensicCard title="Affected Issuers" value="Observed cohort" caption={`${detail.window_attempts} attempts`} /><ForensicCard title="Transaction Sample" value={`${detail.window_failed} failures`} caption={`Average ${formatCurrency(detail.avg_amount)}`} /><ForensicCard title="Confidence Signals" value={`${Math.round((detail.top_confidence || 0) * 100)}%`} caption="Baseline deviation + failure cluster" /></div>{event && <small className="evidence-moment">Captured for: {event.event}</small>}</section>; }

function DecisionSummary({ detail }) { if (!detail) return null; const top = detail.hypotheses?.[0]; return <section className="decision-summary panel"><div className="section-kicker"><FileClock size={14} /> DECISION SUMMARY</div><h3>Reasoning Receipt</h3><div className="receipt-grid"><span>DIAGNOSIS<strong>{CODE_LABEL[top?.code] || "Under review"}</strong></span><span>CONFIDENCE<strong>{Math.round((detail.top_confidence || 0) * 100)}%</strong></span><span>EVIDENCE SOURCES<strong>Gateway latency · Failure cluster · Baseline</strong></span><span>RECOVERY POLICY<strong>{detail.recommendation?.action || detail.chosen_action || "Operator review"}</strong></span><span>BOUNDARIES APPLIED<strong>Bounded retries · Reversible action</strong></span><span>OPERATOR DECISION<strong>{detail.status === "approved" ? "Approved by Operator OP" : "Awaiting operator decision"}</strong></span></div></section>; }

function RecoveryReceipt({ detail, demo, doAction }) { if (!detail) return null; const amount = demo?.caseId === detail.id && demo.progress === 1 ? demo.recoveredAmount : detail.recovered_amount || 0; const tx = demo?.caseId === detail.id && demo.progress === 1 ? demo.recoveredTx : detail.recovered_tx || 0; return <section className="recovery-receipt panel"><div className="section-kicker"><CheckCircle2 size={14} /> RECOVERY RECEIPT</div><h3>{amount ? "Recovery Complete" : "Recovery outcome pending"}</h3><strong className="receipt-amount">{formatCurrency(amount)} recovered</strong><div className="receipt-metrics"><span>{tx} Transactions Retried</span><span>{detail.recommendation?.expected_recovery_rate ? Math.round(detail.recommendation.expected_recovery_rate * 100) : 0}% Recovery Rate</span><span>Execution Time 11 seconds</span><span>Status {amount ? "Successful" : "Pending"}</span></div><div className="receipt-identifiers"><span>IMMUTABLE AUDIT ID <b>RZP-{detail.id}-AUDIT</b></span><span>REPLAY ID <b>REPLAY-{detail.id}</b></span><span>POLICY VERSION <b>v2.4</b></span></div><div className="receipt-actions"><button className="action-button demo-button" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}><Play size={13} />Replay Investigation</button><button className="action-button ghost"><FileClock size={13} />Export Audit Log</button>{detail.status === "approved" && <button className="action-button ghost" onClick={() => doAction(detail.id, "revert")}><RotateCcw size={13} />Mark Ineffective / Revert</button>}</div></section>; }

// Legacy renderer retained for the existing audit card component.
// eslint-disable-next-line no-unused-vars
